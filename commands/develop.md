...
5. Updates `tickets/<active>/state.json` milestone status to `developing` then `developed`.
6. Appends an entry to `tickets/<active>/timeline.jsonl` for every meaningful action (object created/changed, transport touched, test generated) — not just at the end.
