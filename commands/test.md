...
4. Update `tickets/<active>/state.json` milestone status (`testing` → `tested`/`blocked`).
5. Append an entry to `tickets/<active>/timeline.jsonl` per test run, including auto-fix attempts.
