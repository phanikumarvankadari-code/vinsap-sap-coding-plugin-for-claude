...
7. Ask whether to publish to **Confluence** (via the `mcp-atlassian` MCP connector) — create a new page (defaults to `documentation.confluence_parent_id` as the parent folder) or update an existing one, attaching the generated file. Either way, add a comment on the ticket's Jira issue with the page URL (skip if the ticket has no linked Jira issue).
