...
5. Package and transport, per `references/transport-guidelines.md` §2a — never auto-select either:
   - **Package**: ask for a `Z*` prefix to search — pre-fill from the ticket's `functional_areas` using `ZMASTER_<AREA>*` (e.g. `SLS` → `ZMASTER_SLS*`) if set. Search via `adt_search_objects` (type `DEVC`), present up to 100 matches for the user to pick, or create a new one (confirmed name) if none fit.
