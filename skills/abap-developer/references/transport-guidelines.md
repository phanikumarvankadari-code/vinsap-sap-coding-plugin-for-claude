...
5. **Transport scope per package** — one transport touches one application's layers (the leaves under a single `ZMASTER_<AREA>_<APP>` namespace — see `package-hierarchy.md`). Cross-application transports require the user's explicit approval and must be named per §1 with `<MODULE>` set to `<AREA>-<APP>`, e.g. `[AI-VSP] SD-OSOSTK | TICKET-… | …`.
