...
description: Use for /vinsap:switch — changes which ticket subsequent VinSAP commands operate on, without creating a new one. Use /vinsap:onboard instead to start a brand new ticket.
---
...
2. Verify `tickets/<TICKET-ID>/` exists. If it doesn't, **don't create it** — tell the user to run `/vinsap:onboard` instead to start it as a new ticket, or check for a typo.
...
- This is the only command that changes `.sdlc/active_ticket.json` outside of `/vinsap:onboard` setting it for a brand-new ticket.
